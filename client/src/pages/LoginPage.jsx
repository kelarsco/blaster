      ...
      <form onSubmit={handleEmailLogin} className="mt-6 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <label htmlFor="login-password" className="block text-sm font-medium text-blaster-fg">
              Password
            </label>
            <Link to="/forgot-password" className="text-xs text-blaster-muted hover:text-blaster-fg">
              Forgot password?
            </Link>
          </div>
          <PasswordInput
        ...
        {error && (
          <p className="text-sm text-red-600">
            ...
            {error.includes('verify your email') && (
              <> <Link to="/verify-email" className="underline">Verify email</Link></>
            )}
          </p>
        )}
