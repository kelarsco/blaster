 ...
 * user-facing workloads (store scans, etc.).
 */
...
/**
 * Pause background jobs while users are actively scanning.
...
export async function waitForUserWorkloadIdle(options = {}) {
  ...
  while (isUserWorkloadActive()) {
    if (Date.now() - start >= maxWaitMs) break;
    await sleep(pollMs);
  }
}

/** Yield one beat when user work is active (call between background HTTP requests). */
export async function yieldToUserWorkload() {
  if (!isUserWorkloadActive()) return;
